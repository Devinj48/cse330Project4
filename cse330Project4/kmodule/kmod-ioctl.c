...
static long kmod_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case BWRITE: {
            ...
            char *kernel_buffer = kmalloc(rw_request.size, GFP_KERNEL);
            if (!kernel_buffer) {
                return -ENOMEM;
            }
            ...
            if (cmd == BREAD) {
                ret = perform_block_operation(REQ_OP_READ, rw_request.data, rw_request.size, kernel_buffer);
            } else {
                if (copy_from_user(kernel_buffer, rw_request.data, rw_request.size)) {
                    kfree(kernel_buffer);
                    return -EFAULT;
                }
                ret = perform_block_operation(REQ_OP_WRITE, kernel_buffer, rw_request.size, NULL);
            }
        ...
        case BWRITEOFFSET: {
            ...
            /* Allocate a kernel buffer to read/write user data */
            char *kernel_buffer = kmalloc(rwoffset_request.size, GFP_KERNEL);
            if (!kernel_buffer) {
                return -ENOMEM;
            }
            ...
            if (cmd == BREADOFFSET) {
                ...
                ret = perform_block_operation(REQ_OP_WRITE, kernel_buffer, rwoffset_request.size, NULL, rwoffset_request.offset);
            }
